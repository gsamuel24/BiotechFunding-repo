...
- **Prediction**: Biotech companies developing treatments in high-interest therapeutic areas, indications, technology types, and stages of development will receive higher funding amounts.
...
> As stated in Gatlin (2025):  
> “Biotech stocks could become more attractive with interest rates coming down and inflation being curbed.”
...
|--------------------------|------------------------------------------------------|----------------------------------------------------------|-------------------------|
| amount                   | Amount of funding in USD                             | Numeric (currency)                                       | Outcome Variable        |
| round                    | Funding round                                        | Category                                                 | Predictor               |
| federal_fund_rate        | Federal fund effective rate at time of funding       | Numeric                                                  | Predictor (macroeconomic) |
| ten_year_yield           | Ten-year treasury yield at time of funding           | Numeric                                                  | Predictor (macroeconomic) |
| lead_investor            | Lead investor for that funding instance              | Category – One-hot encoded (includes Unknown)            | Predictor               |
| lead_investor_type       | Lead investor type (e.g., private equity, corp vc)   | Category – One-hot encoded (includes Unknown)            | Predictor               |
| stage_at_approval        | Drug development stage at time of funding            | Category – One-hot encoded (includes Unknown)            | Predictor               |
| primary_ta               | Primary therapeutic area of the company              | Category – One-hot encoded                               | Predictor               |
| primary_indication       | Primary indication of the company                    | Category – One-hot encoded                               | Predictor               |
...
| location                 | Location hub (e.g., Boston, San Francisco)           | Category – One-hot encoded (includes Unknown)            | Predictor               |
...
| total_raised_all_rounds  | Amount USA raised in all funding rounds              | Numeric (currency)                                       | Predictor               |
...
### Models Used:
1. **Linear Regression** – Baseline model to assess basic relationships
2. **Random Forest Regressor** – For capturing complex patterns and feature importance
3. **Feedforward Neural Network**  
   - Simple version  
   - Advanced version (dropout, batch norm, regularization)
4. **K-Means Clustering** – To group companies based on traits and funding
5. **PCA (Principal Component Analysis)** – Dimensionality reduction + regression
...
**Tools**: All work will be done in **Python**.  
**Project Repo**: [GitHub Project Link](http://github.com/gsamuel24)
